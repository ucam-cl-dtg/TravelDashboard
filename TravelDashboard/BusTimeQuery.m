...
-(void) makeQueryWithIndex:(int)index {
    ...
    // Get bus data.
    NSString *requestString = [NSString stringWithFormat:@"http://api.stride-project.com/transportapi/7c60e7f4-20ff-11e3-857c-fcfb53959281/bus/stop/%@/%@/%@/timetable", self.busStop, self.date, self.time];
    NSLog(@"%@", requestString);
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    {
        [self.delegate busTimeQueryFailedWithIndex:_index];
    } else {
        ...
        [self.delegate busTimeQueryWithIndex:_index returnedTimetableTimes:aimed];
    }
}
